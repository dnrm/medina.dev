    ...
    return (
        <Container h={"20vh"} mb={16}>
            <Flex justifyContent="start" alignItems="center" textColor="gray.500">
                ...
                { playing ? (
                    <Text>{playing?.isPlaying == true ? <StyledLink href={playing.songUrl} target="_blank" rel="noreferrer">{playing.title}</StyledLink> : "Not playing"} - { playing?.isPlaying == true ? playing.artist : 'Spotify'}</Text>
                ) : null}
            </Flex>
            <Grid
                pt={8}
